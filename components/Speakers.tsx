...
function SpeakerCard({
  ...
  >
    ...
    <path d="M13 5l7 7-7 7" />
      ...
      {entry.speaker && (
        <p className="text-base sm:text-lg font-semibold text-[#17351E] break-words">{language === "kk"
          ? entry.speakerKk ?? entry.speaker
          : entry.speaker}</p>
      )}

      {entry.org && <p className="mt-2 text-sm sm:text-base text-[#6D7568] break-words">{language === "kk" ? entry.orgKk ?? entry.org : entry.org}</p>}
